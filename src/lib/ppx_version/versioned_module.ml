...
let mk_all_version_tags_type_decl =
  let add_bin_io ~loc attrs =
    let (module Ast_builder) = Ast_builder.make loc in
    let open Ast_builder in
    let payload idents =
      let payload = pstr_eval (pexp_tuple idents) [] in
      PStr [ payload ]
    in
    let rec go attrs =
      match attrs with
      ...
      | attr :: tl -> (
          let idents =
            Ast_pattern.(
              attribute ~name:(string "deriving")
                ~payload:(single_expr_payload __))
          in
          match parse_opt idents loc attr (fun l -> Some l) with
          | None ->
              attr :: go tl
          | Some args ->
              let args =
                ...
                | _ ->
                    [ args ]
              in
              modify_attr_payload attr (payload ([%expr bin_io] :: args)) :: tl
          )
    in
    go attrs
  in
  ...
  let extra_stris =
    ...
    let all_version_tag_modules =
      if not all_version_tagged then []
        ...
        let typ_decl = mk_all_version_tags_type_decl#structure_item stri in
        let t =
    ...
    let top_version_tag_modules =
      if not top_version_tag then []
        ...
        [ pstr_module
            (module_binding
               ~name:(some_loc (Located.mk with_top_version_tag_module))
               ~expr:
                 (pmod_structure
...
let convert_module_decls signature =
  ...
  in
  let convert no_toplevel_latest
  ...
  in
  (sig_attrs, List.fold signature_no_attrs ~init ~f:(convert no_toplevel_latest))
                ...
                if top_version_tag then
                  [ [%sigi:
                      val bin_read_top_tagged_to_latest :
                           Bin_prot.Common.buf
                        -> pos_ref:int ref
                        -> Core_kernel.Or_error.t Latest.t]
                  ]
                else []
              in
